...
### Finding 13: Floor Indicator Breakdown — DEFERRED

Floor indicator accuracy (I1-I5 individually) requires its own dedicated session. Preliminary production analysis on 27 games raised methodological concerns: the original WNBA backtest (106 games) measured end-of-game indicator-leader accuracy, while production snapshots capture mid-game indicators that are noisier and use a different data source (BDL vs SR). The 27-game sample is also too small to draw conclusions that could conflict with the validated backtest. Floor TIER win rates on production are clean (DOMINANT ≥0.90 = 95.7%), confirming the composite works regardless of individual indicator behavior.
