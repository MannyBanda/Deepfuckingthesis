...
function computeFullCPTrend(checkpoints, bwcTeam) {
  ...
  const firstAvg = bwcCPs.slice(0, mid).reduce((s, cp) => s + cp.floor, 0) / mid;
  const secondAvg = bwcCPs.slice(mid).reduce((s, cp) => s + cp.floor, 0) / (bwcCPs.length - mid);
  const delta = secondAvg - firstAvg;
  let direction;
  if (delta > 0.04) direction = 'RISING';
  else if (delta < -0.04) direction = 'DECLINING';
  else direction = 'FLAT';
  return { direction, delta: Math.round(delta * 1000) / 1000, floors: bwcCPs.map(cp => cp.floor), margins: bwcCPs.map(cp => cp.margin), count: bwcCPs.length, firstAvg: Math.round(firstAvg * 1000) / 1000, secondAvg: Math.round(secondAvg * 1000) / 1000 };
}
...
function computeFloorMarginSignal(checkpoints, bwcTeam, currentFloor, currentBwcMargin) {
  ...
  const fDelta = f2 - f1, mDelta = m2 - m1;
  let floorTrend = fDelta > 0.03 ? 'RISING' : fDelta < -0.03 ? 'DECLINING' : 'FLAT';
  let marginTrend = mDelta > 1.5 ? 'GROWING' : mDelta < -1.5 ? 'SHRINKING' : 'FLAT';
  let signal = 'ALIGNED';
...
function computeConvictionTrend(checkpoints, bwcTeam) {
  ...
  let degradePoint = null;
  let maxRank = RANK[tiers[0]] || 0;
  for (let ci = 1; ci < tiers.length; ci++) {
    let r = RANK[tiers[ci]] || 0;
    if (r < maxRank && !degradePoint) degradePoint = bwcCPs[ci].label;
...
Erosion: ${ctx.alertType === 'POSITION_OPEN' ? ctx.erosionLevel : (ctx.meanErosionLevel || ctx.erosionLevel)} (${ctx.alertType === 'POSITION_OPEN' ? 'peak-anchored' : 'mean-anchored'})${ctx.peakErosionLevel && ctx.peakErosionLevel !== (ctx.alertType === 'POSITION_OPEN' ? ctx.erosionLevel : (ctx.meanErosionLevel || ctx.erosionLevel)) ? ' | Peak erosion: ' + ctx.peakErosionLevel + ' (reference only)' : ''}
...
function assembleContextPackage(snap, cr, lt, erosion, bwcState, v2Alerts, allSnaps, idx, hA, aA, useMonitor = false, storedLT = null, positionClosed = false) {
  ...
  return {
    ...
    // Position health — compute mean erosion + new signals
    peakFloor: erosion.peakFloor,
    peakDelta: erosion.peakDelta,
    ...(() => { const me = computeMeanErosion(lt, cr); return { meanFloor: me.meanFloor, meanDelta: me.meanDelta, meanErosionLevel: me.level }; })(),
    erosionLevel: erosion.level, // prompt routes PO=peak vs others=mean via alertType check
    peakErosionLevel: erosion.level,
    ...
    fullCPTrend: storedLT?.bwc_fired ? computeFullCPTrend(storedLT?.checkpoints || [], storedLT.bwc_fired.team) : null,
    floorMarginSignal: (() => {
      if (!storedLT?.bwc_fired) return null;
      const bwcT = storedLT.bwc_fired.team;
      const bwcIsH = bwcT === hA;
      const bwcM = bwcIsH ? (cr.homePts - cr.awayPts) : (cr.awayPts - cr.homePts);
      return computeFloorMarginSignal(storedLT?.checkpoints || [], bwcT, cr.floor, bwcM);
    })(),
    convictionTrend: storedLT?.bwc_fired ? computeConvictionTrend(storedLT?.checkpoints || [], storedLT.bwc_fired.team) : null,
