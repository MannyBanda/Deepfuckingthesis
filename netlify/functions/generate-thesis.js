...
function buildNCAAMBSystemPrompt() {
  return [
    ...
    '- NET RANKINGS are the primary strength-of-schedule metric. Weight NET rank + quad records (Q1-Q4 win-loss) heavily for matchup quality.',
    '- Q1 wins (vs top-30 NET home / top-75 away) indicate ability to compete with elite opponents.',
    '- Quad records reveal schedule strength far better than raw W-L record.',
    '- Conference strength context: mid-major vs power conference matters for opponent quality baseline.',
    '- No daily injury report — injuries come from team profile roster status only.',
    '- No depth chart endpoint — derive rotation quality from minutes distribution in season stats.',
    '- No statistical splits — season averages are the only baseline.',
    '- No clutch data, no tracking data (catch-and-shoot/pull-up baselines unavailable).',
    '',
    'AVAILABILITY: Derive from team profile roster. Players with status OUT/INJ/SUS are unavailable. No separate injury report exists.',
    '',
    ...
    'MATCHUP QUALITY: [Quad X game for both teams — indicate significance]',
    '\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501',
    'AVAILABILITY',
    '[TEAM] [count] players available',
    '[TEAM] OUT: [names with positions and reasons from profile]',
    '',
    'REST [TEAM A] X day | [TEAM B] X day',
    '\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501',
    'CONTROL SCORE: [Team] [X.XX] \u2014 [Verdict]',
    ...
    'ML THRESHOLD: [details]',
    '',
    'ENTRY      [description — reference halves not quarters]',
    'PASS       [description]',
    '',
    'WATCH',
    '1. [item — reference H1/H2 for timing]',
    '2. [item]',
    '3. [item]',
  ].join('\n');
}
...
exports.handler = async function(event) {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // NCAAMB PATH — simplified pre-compute, NCAAMB-specific prompt
    // ══════════════════════════════════════════════════════════════════════════
    if (league === 'ncaamb') {
      // Pre-compute: Pythagorean only (SIA/redistribution/SRM/BHV not applicable without depth chart + daily injuries)
      var pyth = computePythagorean(analytical.standings, homeAlias, awayAlias);

      // Build simplified pre-computed section
      var ncaaPreComputed = '=== PRE-COMPUTED STRUCTURAL ASSESSMENT (NCAAMB) ===\n';

      // Pythagorean
      ncaaPreComputed += '\nPYTHAGOREAN EXPECTATION:\n';
      if (pyth.home) ncaaPreComputed += homeAlias + ': ' + pyth.home.actual + '-' + pyth.home.losses + ' (expected ' + pyth.home.expected.toFixed(0) + '-' + (pyth.home.gp - pyth.home.expected).toFixed(0) + ', delta ' + (pyth.home.delta >= 0 ? '+' : '') + pyth.home.delta + ')' + (pyth.home.label ? ' — ' + pyth.home.label : '') + '\n';
      else ncaaPreComputed += homeAlias + ': (not found in standings)\n';
      if (pyth.away) ncaaPreComputed += awayAlias + ': ' + pyth.away.actual + '-' + pyth.away.losses + ' (expected ' + pyth.away.expected.toFixed(0) + '-' + (pyth.away.gp - pyth.away.expected).toFixed(0) + ', delta ' + (pyth.away.delta >= 0 ? '+' : '') + pyth.away.delta + ')' + (pyth.away.label ? ' — ' + pyth.away.label : '') + '\n';
      else ncaaPreComputed += awayAlias + ': (not found in standings)\n';

      // Availability from profiles (NCAAMB has no daily injuries endpoint)
      ncaaPreComputed += '\nAVAILABILITY (from team profiles — no daily injury report for NCAAMB):\n';
      ['home', 'away'].forEach(function(side) {
        var alias = side === 'home' ? homeAlias : awayAlias;
        var profile = analytical[side + 'Profile'];
        if (!profile || !profile.players) { ncaaPreComputed += alias + ': (roster unavailable)\n'; return; }
        var out = [], gtd = [], active = 0;
        (profile.players || []).forEach(function(p) {
          var st = (p.status || '').toUpperCase();
          var name = p.full_name || '?';
          var pos = p.primary_position || p.position || '?';
          if (st === 'OUT' || st === 'INJ' || st === 'SUS' || st === 'NWT') {
            var injDesc = '';
            if (p.injuries && Array.isArray(p.injuries) && p.injuries.length > 0) {
              injDesc = ' — ' + (p.injuries[0].desc || p.injuries[0].comment || '');
            }
            out.push(name + ' [' + pos + '] \u274C ' + st + injDesc);
          } else if (st === 'D-LEAGUE' || st === 'TWO-WAY') {
            // skip
          } else {
            active++;
            if (p.injuries && Array.isArray(p.injuries) && p.injuries.length > 0) {
              var li = p.injuries[0];
              var liSt = (li.status || '').toUpperCase();
              if (liSt === 'DAY-TO-DAY' || liSt === 'QUESTIONABLE' || liSt === 'DOUBTFUL' || liSt === 'PROBABLE') {
                gtd.push(name + ' [' + pos + '] \u26A0\uFE0F ' + liSt + ' — ' + (li.desc || li.comment || ''));
              }
            }
          }
        });
        ncaaPreComputed += alias + ': ' + active + ' players available\n';
        if (out.length) ncaaPreComputed += alias + ' OUT: ' + out.join(' | ') + '\n';
        if (gtd.length) ncaaPreComputed += alias + ' GTD: ' + gtd.join(' | ') + '\n';
        if (!out.length && !gtd.length) ncaaPreComputed += alias + ': FULL STRENGTH\n';
      });

      var ncaaSystemPrompt = buildNCAAMBSystemPrompt();

      var ncaaUserPrompt = 'Build a complete pre-game thesis for this NCAAMB matchup.\n\n' +
        ...
        (sections.odds || '') + '\n\n' +
        'NCAAMB CONTEXT: This is college basketball. Two 20-minute halves. 5 personal fouls = disqualification. 30-second shot clock. ~65-70 possessions per game. No depth chart, no splits, no clutch data, no tracking baselines. Derive rotation from minutes distribution in season stats. NET rankings + quad records are the primary strength metric — use them heavily.\n\n' +
        'Output the compact thesis format.';

      var ncaaResp = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 4000, system: ncaaSystemPrompt, messages: [{ role: 'user', content: ncaaUserPrompt }] }),
      });

      if (!ncaaResp.ok) {
        var ncaaErrText = await ncaaResp.text();
        return { statusCode: ncaaResp.status, headers: headers, body: JSON.stringify({ error: 'Anthropic ' + ncaaResp.status + ': ' + ncaaErrText.substring(0, 300) }) };
      }

      var ncaaData = await ncaaResp.json();
      var ncaaThesis = ncaaData.content.filter(function(b) { return b.type === 'text'; }).map(function(b) { return b.text; }).join('\n');

      return {
        statusCode: 200, headers: headers,
        body: JSON.stringify({
          thesis: ncaaThesis, usage: ncaaData.usage,
          preComputed: {
